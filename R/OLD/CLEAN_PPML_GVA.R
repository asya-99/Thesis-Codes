...
# Fossil row background — fgFill is the correct parameter for solid fills
fos_bg   <- createStyle(fgFill="#FFF3CD")

# Non-fossil coefficient styles
neg_norm <- createStyle(fontColour="#C0392B", textDecoration="bold")
pos_norm <- createStyle(fontColour="#1D6A40", textDecoration="bold")

# Fossil coefficient styles — fgFill matches the row background
...
add_sheet <- function(wb, name, data) {
  ...
  for(i in seq_len(nrow(data))) {
    ...
    # Step 1: amber background across whole row for fossil industries
    if(is_fos)
      addStyle(wb, name, fos_bg, rows=i+1,
               cols=1:ncol(data), gridExpand=TRUE, stack=FALSE)

    # Step 2: font colour on coefficient columns (bgFill matches row colour)
    for(cc in coef_cols) {
