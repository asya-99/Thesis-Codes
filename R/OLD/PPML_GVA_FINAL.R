...
df_gva_oecd <- df_gva[df_gva$REF_AREA %in% oecd_iso3, ]
df_gva_eu   <- df_gva[df_gva$REF_AREA %in% eu_iso3, ]
df_gva_nonoecd <- df_gva[!df_gva$REF_AREA %in% oecd_iso3, ]
df_gva_noneu   <- df_gva[!df_gva$REF_AREA %in% eu_iso3, ]

cat("=== SAMPLE SIZES ===\n")
cat("PUSH — GVA OECD countries:     ", length(unique(df_gva_oecd$REF_AREA)),
    "countries |", nrow(df_gva_oecd), "rows\n")
cat("PUSH — GVA EU countries:       ", length(unique(df_gva_eu$REF_AREA)),
    "countries |", nrow(df_gva_eu), "rows\n")
cat("PULL — GVA non-OECD countries: ", length(unique(df_gva_nonoecd$REF_AREA)),
    "countries |", nrow(df_gva_nonoecd), "rows\n")
cat("PULL — GVA non-EU countries:   ", length(unique(df_gva_noneu$REF_AREA)),
    "countries |", nrow(df_gva_noneu), "rows\n")
cat("PULL — GVA all countries:      ", length(unique(df_gva$REF_AREA)),
    "countries |", nrow(df_gva), "rows\n")
...
df_gva_oecd <- add_lags(df_gva_oecd,   c("EPS", "EPI"))
df_gva_eu      <- add_lags(df_gva_eu,      c("EPS", "EPI"))
df_gva_nonoecd <- add_lags(df_gva_nonoecd, "EPI")
df_gva_noneu   <- add_lags(df_gva_noneu,   "EPI")
df_gva         <- add_lags(df_gva,          "EPI")



cat("\n=== LAG AVAILABILITY ===\n")
cat("EPS lag1 (OECD):", sum(!is.na(df_gva_oecd$EPS_lag1)), "\n")
cat("EPI lag1 (OECD):", sum(!is.na(df_gva_oecd$EPI_lag1)), "\n")
cat("EPI lag1 (non-OECD):", sum(!is.na(df_gva_nonoecd$EPI_lag1)), "\n")
