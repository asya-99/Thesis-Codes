# =============================================================================
# OBS_VALUE (EPS) x EXP — Two-Way Fixed Effects Panel Regression
# Controls: GDP, TRADE
# Includes contemporaneous (t) and 1-year lag (t-1) coefficients
# Output: EPS_TWFE_results.xlsx with one sheet
# =============================================================================
...
# 1. LOAD DATA
# =============================================================================
df <- read_excel("./COMPLETE.xlsx", sheet = "Sheet1")
...
exp_cols <- names(df)[grepl("_(Domestic|Foreign)$", names(df))]
for (col in exp_cols) df[[col]] <- as.numeric(df[[col]])

cat("Rows:", nrow(df), "| Countries:", length(unique(df$REF_AREA)),
    "| Years:", length(unique(df$TIME_PERIOD)), "\n")

# =============================================================================
# 2. CREATE 1-YEAR LAG OF OBS_VALUE (within country)
# =============================================================================
...
df$OBS_VALUE_lag1 <- lag_within(df$OBS_VALUE, df$REF_AREA)
cat("Rows with lagged OBS_VALUE:", sum(!is.na(df$OBS_VALUE_lag1)), "\n")
...
# Model: log(EXP_it) ~ policy_it + log(GDP_it) + TRADE_it + country_FE + year_FE
...
# =============================================================================
industry_codes <- unique(sub("_(Domestic|Foreign)$", "", exp_cols))
...
run_model <- function(policy_var) {
  ...
  for (ind in industry_codes) {
    ...
    make_row <- function(res, exp_type) {
      if (is.null(res)) return(NULL)
      data.frame(
        Industry    = ind, Description = label, exp_type = exp_type,
...
cat("\nRunning contemporaneous model: OBS_VALUE(t)...\n")
res_t  <- run_model("OBS_VALUE")

cat("Running lagged model: OBS_VALUE(t-1)...\n")
res_l1 <- run_model("OBS_VALUE_lag1")
...
build_table <- function(res_t, res_l1) {
  ...
  for (ind in inds) {
    ...
    for (gt in c("Domestic", "Foreign", "Total")) {
      t_row  <- res_t [res_t$Industry  == ind & res_t$exp_type  == gt, ]
      l_row  <- res_l1[res_l1$Industry == ind & res_l1$exp_type == gt, ]
      ...
      row[[paste0("n_t_",     gt)]] <- if (nrow(t_row)  > 0) t_row$n_obs        else NA
      ...
      row[[paste0("n_l1_",    gt)]] <- if (nrow(l_row) > 0) l_row$n_obs         else NA
    }
    out <- rbind(out, row)
  }
...
cat("\n--- OBS_VALUE(t) vs OBS_VALUE(t-1) coefficients ---\n")
...
# Main sheet: side-by-side t vs t-1 for all three EXP types
coef_cols_main <- c("coef_t_Domestic","coef_l1_Domestic",
                    "coef_t_Foreign","coef_l1_Foreign",
                    "coef_t_Total","coef_l1_Total")
add_sheet(wb, "OBS_VALUE TWFE", side_by_side, coef_cols_main)
...
add_sheet(wb, "OBS_VALUE Fossil", fossil_out, coef_cols_main)
...
add_sheet(wb, "OBS_VALUE Full Detail", full_detail, "coef_policy")

out_path <- "/Users/asyadachi/gent/R_codes/EPS_PUSH_results.xlsx"
saveWorkbook(wb, out_path, overwrite = TRUE)
cat("\nSaved to:", out_path, "\n")

# =============================================================================
# KEY:
# Model: log(EXP_it) ~ OBS_VALUE_it + log(GDP_it) + TRADE_it + country_FE + year_FE
# coef_t      — effect of OBS_VALUE this year on EXP this year
# coef_l1     — effect of OBS_VALUE last year on EXP this year
